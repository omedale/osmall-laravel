  ...
  $.paletteColorPicker = function( el, options ) {
    ...
    plugin.init = function() {
      ...
      .on( click_handler, 'span.swatch', function(e){
		  ...
		  console.log(col);
			...
			for(var jj = 1; jj <= parseInt($("#colors_id").val()); jj++){
				...
				if(css != null){
					...
					var thiscolor = rgbToHex1(parseInt(commas[0].replace(/ /g,'')),parseInt(commas[1].replace(/ /g,'')),parseInt(last[0].replace(/ /g,'')));
					colorshex[numn] = thiscolor;
					if(colorshex.indexOf(col) == -1){
        ...
        if ( $(e.target).is('.clear') || repeat ) {
			if(repeat){
				toastr.error("Warning: Colors can't repeat");
			}
			$button.removeAttr('style');
			col = '';
        } else {
