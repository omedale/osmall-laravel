					...
					@if(isset($product) && !empty($product))
						...
						<tr>
							...
							<td class="text-center">
								<?php
									$product_id =IdController::nTp($productList->id);
								?>
								{{$product_id}}
							</td>
							<?php 
							...
							?>
							<td>{{$pname}}</td>							
							<td class="text-center"><a class="price_details" rel="{{ $productList->id }}" target="_blank"  href="{{URL::to('/')}}/admin/master/productterm/{{ $productList->id }}">  Details </a></td>																	
						</tr>
						@endforeach
					@endif
					</tbody>
