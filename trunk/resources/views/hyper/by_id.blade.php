  ...
  border-color: #00FF11;
	...
	<div class="row">
		<div class="col-sm-11 col-sm-offset-1">
			...
			<h1>Hyper</h1>
			<hr>
			<h3>{{!empty($cat_name) ? $cat_name->description : ""}} <small><span id="ow_product_count">{{count($ow_product)}}</span> result(s)</small></h3>
			{{-- php code --}}
			 <?php if(!empty($ow_product)){ $i = 1;} ?> 
			{{-- HyperBoxes 4x4 --}}
			<div class="row">
				  <div class="col-sm-12">
				    <div class="row">
							...
							?>							
						     <div class="col-md-4">
						        <div class="well owarehouseform">
									...
									<?php
									echo $dDiff->format("%r") == '-' ? '<div></div>' :'Time Left: '.($dDiff->y >0 ? $dDiff->y ." y ":"").($dDiff->m >0 ? $dDiff->m ." mo ":""). $dDiff->d ." days" . $dDiff->h ." hrs and ". $dDiff->i ." mins "; ?></mark></center></span>
									...
									{!! Form::open(array('role'=>'role','class'=>'owarehouseformd', 'style'=>'margin-top: 7px;')) !!}
										...
										<p class="pull-right" style="vertical-align: middle;">
											<button type="button" class="btn btn-green btn-number quantity" style="vertical-align: middle; width:28px; height: 28px; padding: 4px;" data-id='+' min="{{$product->owarehouse_moqperpax}}" max="{{$left}}">
												<span class="glyphicon glyphicon-plus"></span>
											</button>												

										   <input type="text" class="order_value text-center" size="4" align="center" style="vertical-align: middle; height: 28px;"  name="quant[2]" value="{{$product->owarehouse_moqperpax}}" min="{{$product->owarehouse_moqperpax}}" max="{{$left}}" >
								   <button type="button" class="btn btn-green btn-number quantity" style="vertical-align: middle; width:28px; height: 28px; padding: 4px;" data-id='-' min="{{$product->owarehouse_moqperpax}}" max="{{$left}}">
													 <span class="glyphicon glyphicon-minus"></span>
											</button>										   	
											<span title="You are buying a Hyper Discount Coupon in order to participate. In the case of unsuccessful sales, the amount will be added to your OpenCredit balance">
...
$(document).ready(function(){
		...
		if((ow_product - ow_count) == 0){
			window.location.replace(JS_BASE_URL + "/owarehouselist");
		}
			...
			if(parseInt(quantity) >= parseInt(moqpax)){
				if((parseInt(quantity) % parseInt(moqpax)) == 0){
					if(canpledge == "1"){
							...
							if(quantity > 0){
								$.ajax({
									...
									success:function(data){
										...
										toastr.info("Your contribution was successful. Product has been added to the Cart. Reloading page.");
									}
									// success: function(data) {								
									// 				if(quantity > 0){
									// 				$.ajax({
									...
									// 						// $('.cart-info').text(data[1] + ' (' + 'RM ' + price * quantity + ' with quantity '+ quantity +') ' + " Successfully added to the cart");
									// 						// if (data[0] < 1)
									// 						// {
						...
						toastr.error("Please login in order to contribute");
					}			 
				} else {
