...
<div class="container" style="margin-top:30px;">
	...
	<h2>Product O-Shop Transfer</h2>
	<h3>Product ID: {{IdController::nP($product->id)}}</h3>
		...
		<table class="table table-bordered" cellspacing="0" width="100%" id="gridmerchant">
			...
			<tbody>
				...
				<tr style="{{$style}}">
					...
					<td class="text-center">
					@if($oshop->single == 0)
						<a href="{{route('oshop.one',['url'=>$oshop->url])}}" target="_blank" data-id="{{$oshop->id }}"> 
						{{IdController::nOshop($oshop->id)}} </a>
					@else
					...
					<td class="text-center">
						<a href="javascript:void(0)" class="view-merchant-modal" data-id="{{$oshop->merchant_id }}"> 
						{{IdController::nM($oshop->merchant_id)}} </a> 
					</td>
					<td class="text-center">
						{{$oshop->company_name}}
					</td>
					<td class="text-center">
						<input type="checkbox" class="selectedoshop" value="{{ $oshop->id }}" />
					</td>
				</tr>
				@endforeach
			</tbody>
				...
				<tfoot>
					<tr>
						...
						@if($product->status == 'deleted')
							<th><button class='btn btn-block btn-danger btn-sm btn-transfer' disabled rel="{{$product->id}}">Transfer</button></th>
						@else
							<th><button class='btn btn-block btn-danger btn-sm btn-transfer' rel="{{$product->id}}">Transfer</button></th>
						@endif
					</tr>
				</tfoot>
			@endif
