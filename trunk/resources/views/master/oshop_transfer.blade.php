...
<div class="container" style="margin-top:30px;">
	...
	<h2>O-Shop Transfer</h2>
	<h3>O-Shop ID: {{IdController::nOshop($oshop->id)}}</h3>
		...
		<table class="table table-bordered" cellspacing="0" width="100%" id="gridmerchant">
			<thead style="background-color: #FF4C4C; color: white;">
				<tr>
					...
					<th class="text-center medium">Brand&nbsp;Name</th>
					<th class="text-center medium">Company&nbsp;Name</th>
					<th class="text-center medium">Select</th>
				</tr>
			</thead>
			<tbody>
				<?php $m = 0; ?>
				@foreach($oshops as $oshopn)
				<?php $m++; ?>
				<?php 
					$style = "";
				?>
				<tr style="{{$style}}">
					...
					<td class="text-center">
						<a href="javascript:void(0)" class="view-merchant-modal" data-id="{{$oshopn->merchant_id }}"> 
						{{IdController::nM($oshopn->merchant_id)}} </a> 
					</td>
					<td class="text-center">
						{{$oshopn->brand_name}}
					</td>
					...
					<td class="text-center">
						<input type="checkbox" class="selectedoshop" value="{{ $oshopn->merchant_id }}" />
					</td>
				</tr>
				@endforeach
			</tbody>
			@if(isset($oshops) and !is_null($oshops))
				<tfoot>
					<tr>
						<th colspan=4></th>
						@if($oshop->status == 'transferred')
							<th><button class='btn btn-block btn-danger btn-sm btn-transfer' disabled rel="{{$oshop->id}}">Transfer</button></th>
						@else
							<?php 
								$countp = DB::table('oshopproduct')->join('product','product.id','=','oshopproduct.product_id')->where('product.status','!=','transferred')->where('product.status','!=','deleted')->where('oshopproduct.oshop_id',$oshop->id)->count();
								//dd($countp);
							?>
							@if($countp == 0)
								<th><button class='btn btn-block btn-danger btn-sm btn-transfer' title='This O-Shop have no product to be transferred' disabled rel="{{$oshop->id}}">Transfer</button></th>
							@else
								<th><button class='btn btn-block btn-danger btn-sm btn-transfer' rel="{{$oshop->id}}">Transfer</button></th>
							@endif
						@endif
					</tr>
				</tfoot>
    ...
    function pad (str, max) {
        ...
        return str.length < max ? pad("0" + str, max) : str;
			...
			$(document).delegate( '#save_notes', "click",function (event) {
				...
				if(oshop == ""){
					toastr.warning("Please, select one Merchant");
				} else {
					var formData = {
						oshop_id: $("#tproduct_id").val(),
						merchant_id: oshop,
					...
					$.ajax({
						type: "POST",
						url: JS_BASE_URL + "/oshop/transfer",
						data: formData,
						success: function (data) {
							console.log(data);
							$('#save_notes').attr('value', "Save Notes");
							$("#myModalNotes").modal('toggle');
							toastr.info('O-Shop Successfully Transferred');
						},
						error: function (error) {
							console.log(error);
