		...
		<table class="table table-bordered" cellspacing="0" width="100%" id="gridmerchant">
			<thead style="background-color: #FF4C4C; color: white;">
				<tr>
					...
					<th class="text-center medium">Sales</th>
					<th class="text-center medium">Transfer</th>						
					<th class="medium text-center" style="background-color:#008000;color:#fff">Status</th>	
				</tr>
			</thead>
			<tbody>
				<?php $m = 0; ?>
				...
				<tr style="{{$style}}">
					...
					<td class="text-center">
						MYR&nbsp;{{number_format($oshop->sales/100,2)}}
					</td>
					...
					<td class="text-center">
						...
						@else
						@if($oshop->single == 0)
							<a href="{{URL::to('/')}}/admin/master/oshop/transfer/{{$oshop->id}}" target="_blank" data-id="{{$oshop->id }}"> Transfer </a>
						@endif
						@endif
					</td>
					...
					<td class="text-center">
						@if($oshop->status == 'transferred')
							Transferred
						@else
							<a class="approval_details" rel="{{ $oshop->id }}" target="_blank"  href="{{route('oshopApproval', ['id' => $oshop->id])}}">{{ucfirst($oshop->status)}}</a>
						@endif
					</td>
				</tr>
				@endforeach
