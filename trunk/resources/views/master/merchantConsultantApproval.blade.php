...
<div class="container" style="margin-top:30px;">
	@include('admin/panelHeading')
		<div class='row'>		
			<div class='col-xs-6'><h2>Merchant Consultant Master</h2></div>
			<div class='col-xs-6'>
				@foreach($merchantsconsultant as $merchantc)
					<a class="btn btn-default role_status_button pull-right add_remark" role="sales_staff" do_status="add_remark" current_role_id="{{$merchantc->ssid}}" style="width: 110px !important;" href="javascript:void(0)"> Add Remark </a>
				@endforeach
			</div>	
		</div>		
		...
		<table class="table table-bordered" cellspacing="0" width="1160px" id="grid4c">
			...
			<tbody>
				...
				<tr>
					...
					<td class="text-center">
						<?php $formatted_merchantc_id = str_pad($merchantc->ssid, 10, '0', STR_PAD_LEFT); ?>
						<!--<a target="_blank" href="{{route('userPopup', ['id' => $merchantc->user_id])}}">[{{$formatted_merchantc_id}}]</a>-->
						<a href="javascript:void(0)" class="view-merchID-modal" data-id="{{$merchantc->user_id }}"> 
				...
				<!--	<td class="text-center">
					...
					<td class="text-center">
						<a rel="{{ $merchantc->ssid }}"
						class="commission" href="javascript:void(0)">
						Details</a>
						<input id="target_{{ $merchantc->ssid }}" type="hidden" value="{{$merchantc->target_merchant}}" />

						<?php
						$revenue = number_format(($merchantc->target_revenue/100),2);
            			?>
            			<input id="revenue_{{ $merchantc->ssid }}" type="hidden" value="{{$currency}}&nbsp;{{$revenue}}" />

						<input id="bonus_{{ $merchantc->ssid }}" type="hidden" value="{{$merchantc->bonus}}%" />
					</td> -->
					...
					<?php
						...
						->where('sales_staffremark.sales_staff_id',$merchantc->ssid)
					...
					?>
						<a href="javascript:void(0)" id="mcrid_{{$merchantc->ssid}}" class="mcrid" rel="{{$merchantc->ssid}}">
							<span title='{{$pfullremark}}'>{{$premark}}</span>
						</a>
					</td> 
					...
					<td>
						<div class="action_buttons">
							<?php
							$approve = new Classes\Approval('sales_staff', $merchantc->ssid);
...
$(document).ready(function () {
    ...
    $(document).delegate( '.mcrid', "click",function (event) {
		...
		$.ajax({
			...
			success: function (data) {
				console.log(data);
				var html = "<table class='table table-bordered' cellspacing='0' width='100%' ><tr style='background-color: #ff6600; color: white;'><th class='text-center'>No</th><th class='text-center'>MC&nbsp;ID</th><th class='text-center'>Status</th><th class='text-center'>Admin&nbsp;User&nbsp;ID</th><th class='text-center'>Remarks</th><th class='text-center medium'>DateTime</th></tr>";
