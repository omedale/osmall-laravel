		...
		<table class="table table-bordered" cellspacing="0" width="100%" id="grid4c">
			<thead style="background-color: #0000ff; color: #fff;">
				<tr>
					...
					@if (Auth::check() && (Auth::user()->hasRole('hr') || Auth::user()->hasRole('adm')))
			...
			<tbody>
				...
				<tr>
					...
					<?php
						$ss = DB::table('sales_staff')->
							where('user_id',$buy->user_id)->count();
						$emp = DB::table('employee')->
							where('user_id',$buy->user_id)->count();
						$buyerr = DB::table("role_users")->where('user_id', $buy->user_id )->where('role_id',2)->count();
						$troles = $ss + $emp + $buyerr;
					?>
					<td style="text-align: center;">
						<a rel="{{ $buy->user_id }}" class="roles"
							href="javascript:void(0)">{{$troles}}</a>
					</td>
					@endif
					<td style="text-align: center;">
						<a rel="{{ $buy->user_id }}" class="likes"
							href="javascript:void(0)">{{$buy->likes}}</a>
					</td>
					
					<td style="text-align: center;">
						<a rel="{{ $buy->email }}" class="others"
							target="_blank"
							href="{{route('buyerOthers',['id'=>$buy->id])}}">
								Details</a>
					</td>
					<td id="status_column" class="text-center">
						<span id="status_column_text">
							<a rel="{{ $buy->email }}" target="_blank"  href="{{route('buyerApproval', ['id' => $buy->id])}}">{{ucfirst($buy->status)}}</a>
						</span>
					</td>
				</tr>
				@endforeach
