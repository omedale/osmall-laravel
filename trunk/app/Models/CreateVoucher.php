...
class CreateVoucher extends Model
{
  ...
  public static function CreateVoucherDetails($VoucherId)
 {
 		$result  = DB::table('voucher')
 		 		 ...
 		 		 ->select('product.*','address.line1','address.line2','address.line3','brand.name as brandname','subcat_level_1.name as subname','category.name as catName')
...
for ($k=0; $k <sizeof($data['bookingDate']); $k++) {
 ...
 public static function product()
 {
 	$result = DB::table('product')
 			...
 			->join('category','product.category_id','=','category.id')
 			->select('product.*','category.name as catName','voucher.id as voucherId')
